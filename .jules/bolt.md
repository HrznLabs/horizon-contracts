...
## 2024-07-09 - [Loop Fusion in FeeDistributor]
**Learning:** In Solidity, when resetting state variables in a mapping (like `guildVolume`) over an array of keys (`guilds`), looping over the array a second time costs significant redundant gas (`O(2N)`).
**Action:** Merge the state reset (`delete guildVolume[guild]`) into the primary loop that processes the data. Caching the value into memory (`uint256 vol = guildVolume[guild]`) before deletion preserves the original data for use within the loop while executing the state modification.
