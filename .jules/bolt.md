...
## 2026-08-25 - FeeDistributor Loop Fusion
**Learning:** Merging the secondary guildVolume reset loop into the primary distribution loop (loop fusion) in `FeeDistributor.sol` eliminates a redundant O(N) iteration, saving gas.
**Action:** Placed the state deletion (`delete guildVolume[guild]`) unconditionally within the merged loop by caching the volume in memory prior to deletion, avoiding any bypasses from conditionals.
