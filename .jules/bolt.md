...
## $(date +%Y-%m-%d) - Loop Fusion in FeeDistributor
**Learning:** Found a case where `guilds` array was iterated twice in `FeeDistributor.sol` (once for transferring, once for deleting states). Consolidating loop operations saves SLOAD and iteration gas.
**Action:** Look for 'loop fusion' opportunities where multiple state updates or transfers on the same array can be consolidated into a single pass.
