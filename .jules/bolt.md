...
## 2026-09-16 - Optimizing accumulators in batch loops
**Learning:** When batch processing multiple items that affect a global accumulator (e.g., totalGlobalXP), caching the storage variable to memory before the loop, updating locally, and writing to storage once after the loop significantly saves gas by replacing N SLOAD/SSTORE operations with 1.
**Action:** Always look for global counters or total trackers inside batch update loops and lift them into local variables outside the loop.
