...
## 2026-09-10 - Cache Array Lengths and Pre-increment
**Learning:** Caching array lengths in memory (especially for storage arrays) rather than reading them on each loop iteration, and using the pre-increment operator (`++i`), are effective gas optimizations in Solidity.
**Action:** Always cache array lengths in memory before loops and use pre-increment operators in loops.
