...
## 2024-05-28 - Missing PaymentRouter Authentication Function in Mission Factory Clones
**Learning:** Factory contracts that deploy minimal proxy (EIP-1167) clones which subsequently need to authenticate to shared routing contracts (like PaymentRouter) must implement reverse-lookup functionality (e.g. `getMissionByEscrow(address)`) if the router relies on it for access control checks. Failing to do so can trap value in the clones because external calls to the router will revert.
**Action:** When auditing factory contracts that deploy escrows or other value-holding clones, always verify that the factory implements all interface methods required by the router or registry contracts that the clones interact with. Check the modifiers on those external systems.
