...
## 2025-06-25 - [Time-Based Refund Blocked by Late Submission]
**Vulnerability:** The `submitProof` function in `MissionEscrow` (and overridden in `DeliveryEscrow`) allowed a performer to submit proof *after* the `expiresAt` timestamp, transitioning the mission state to `Submitted`. This prevented the poster from successfully calling `claimExpired()`, which required the state to not be `Submitted`.
**Learning:** State-transition functions (like `submitProof`) that move a time-locked contract into a "protected" state (like `Submitted`, which blocks refunds) must strictly enforce the same deadlines as the refund mechanisms (like `claimExpired`). Otherwise, a malicious party can intentionally wait until after the deadline and submit a late proof to hold the funds hostage or force an unnecessary dispute.
**Prevention:** Always verify that all possible state transitions out of the initial or waiting states (e.g., `Open`, `Accepted`) explicitly check the expiration constraints (e.g., using a `notExpired` modifier). Re-check overridden functions to ensure modifiers from the parent aren't lost.
