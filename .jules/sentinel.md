...
## 2024-05-28 - Missing `getMissionByEscrow` in Factory Escrow Causes Settlement Failure
**Vulnerability:** `DeliveryMissionFactory` lacks `getMissionByEscrow`, causing `PaymentRouter`'s `_isFactoryEscrow` (which relies on `try IMissionFactory(factory).getMissionByEscrow(caller)`) to fail when an escrow clone attempts to settle payments. This bricks settlements for delivery escrows.
**Learning:** Try-catch statements on specific interface methods will swallow failures (reverting the try-block and entering the catch block) if the target contract lacks the method, potentially masking crucial logic bugs (in this case, authorization logic) rather than emitting clear reverts.
**Prevention:** Ensure all factory contracts that deploy authorized escrow clones conform strictly to the expected interface expected by central routers, and have comprehensive integration tests for end-to-end user flows (like settling a delivery mission) to catch missing method signatures early.
