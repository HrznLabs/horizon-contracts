...
## 2026-06-12 - [DoS via Missing Caller Validation]
...
## 2024-05-18 - PaymentRouter Settlement Failure for Delivery Missions
**Vulnerability:** Delivery missions cannot be settled because `PaymentRouter` fails to verify the caller as a valid escrow. `PaymentRouter._isFactoryEscrow` calls `IMissionFactory(missionFactory).getMissionByEscrow(caller)`. However, `DeliveryMissionFactory` lacks the `getMissionByEscrow` method and the corresponding mapping. When an escrow clone attempts to settle, the call reverts because the factory contract does not support the expected function signature, causing `PaymentRouter` to reject the settlement with `OnlyMissionEscrow()`.
**Learning:** Factory contracts that spawn escrows and share a single `PaymentRouter` must implement identical interfaces for escrow verification. A mismatch in the factory interface completely breaks the downstream payment routing, locking funds in the escrow.
**Prevention:** Ensure that all factory contracts (e.g., `MissionFactory` and `DeliveryMissionFactory`) implement the full interface expected by auxiliary contracts like `PaymentRouter`. Alternatively, `PaymentRouter` could check for multiple known factories or use a standardized registry.
