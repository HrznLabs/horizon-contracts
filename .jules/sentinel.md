...
## 2026-09-22 - DeliveryEscrow PaymentRouter Auth Revert
**Learning:** Factory contracts that deploy authorized escrows (e.g., `DeliveryMissionFactory.sol`) must implement `getMissionByEscrow(address)` to allow `PaymentRouter._isFactoryEscrow()` to authenticate clones correctly and prevent settlement failures.
**Action:** Verify all factory contracts interacting with `PaymentRouter` strictly implement `IMissionFactory` or the required subset of methods for authorization.
