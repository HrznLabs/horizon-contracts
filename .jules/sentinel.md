...
## 2026-04-28 - Missing getMissionByEscrow breaks DeliveryEscrow settlement
**Vulnerability:** `DeliveryMissionFactory` lacks `getMissionByEscrow(address)`. `PaymentRouter`'s `_isFactoryEscrow` check relies on this function, meaning cloned `DeliveryEscrow`s are not authorized to call `settlePayment()`.
**Learning:** When creating alternative factories for escrow clones (like `DeliveryMissionFactory` parallel to `MissionFactory`), they must fully conform to the implicit interface expected by centralized authorization contracts like `PaymentRouter`.
**Prevention:** Always implement `getMissionByEscrow` in factory contracts that deploy authorized escrows to ensure clones can successfully route payments.
