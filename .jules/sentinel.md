...
## 2024-05-24 - [MissionEscrow Expiration Bypass]
**Vulnerability:** Performers could submit a proof of completion for an escrow even after the `expiresAt` deadline had passed. This could be used to front-run a poster's `claimExpired()` transaction, moving the escrow to `Submitted` state and denying the poster their time-locked refund, potentially forcing them into a dispute.
**Learning:** In Solidity, when time-locked constraints (like an expiration) are meant to restrict state transitions (like `submitProof`), the function must explicitly enforce the time boundary constraint (using a modifier like `notExpired`). Additionally, when a child contract (like `DeliveryEscrow`) overrides a function from the parent contract (like `MissionEscrow`), modifiers from the parent function are *not* automatically inherited.
**Prevention:** Always verify that any function causing a state transition out of an open state explicitly checks expiration deadlines if a timeout mechanism exists. When overriding functions, ensure all relevant state-checking modifiers are reapplied.
