...
## 2024-11-20 - Missing Interface in Value-at-Risk Factory\n**Learning:** Factory contracts that spawn autonomous clones (escrows) and require reverse-lookup for authentication (like `getMissionByEscrow`) in a centralized payment router will silently fail authorization checks if the factory fails to implement the required method. The `try/catch` block in the router catches the revert and returns false, leading to a silent failure pattern.\n**Action:** Always verify that newly introduced factories in an established system fully implement all reverse-lookup methods required by central routers.
