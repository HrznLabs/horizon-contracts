...
## 2025-07-04 - [Late Submission Blocking Refund]
**Vulnerability:** The `submitProof` function in `MissionEscrow.sol` lacked the `notExpired` modifier, allowing a performer to submit work after the mission deadline had passed. This transitioned the mission state to `Submitted`, which permanently blocked the poster from reclaiming their funds via `claimExpired` (which reverts if the state is `Submitted`).
**Learning:** Time-based deadlines must be enforced consistently on all state transitions that can lock funds or block intended time-based resolution paths (like refunds). A missing expiration check on a forward state transition can be weaponized to prevent an expiration-based backward transition.
**Prevention:** When implementing state machines with time-based elements (like expiration), visualize the state machine and ensure that forward-progressing actions (like submitting proof) are strictly bounded by the same time limits that unlock fallback actions (like claiming expired funds).
