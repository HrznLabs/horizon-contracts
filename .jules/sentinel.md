...
## 2026-03-22 - Found Privilege Escalation Bug
**Vulnerability:** Found a privilege escalation missing an explicit check preventing members with the `ADMIN_ROLE` from being removed by lower-tier roles (like `OFFICER_ROLE`).
**Learning:** Implementing defense in depth by correctly enforcing least privilege through proper role validation.
**Prevention:** In functions mapping access levels based on role management, double-check that every restricted action includes explicit verification of all allowed or disallowed higher-level roles.
