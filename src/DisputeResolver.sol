...
contract DisputeResolver is IDisputeResolver, Ownable, ReentrancyGuard {
        ...
        returns (uint256 disputeId)
    {
        ...
        // Set DDR deposit deadline using the guild-level timeout override if present.
        // Pass params.guild so guilds with custom DDR windows get the correct deadline.
        uint256 timeout = _resolveDDRTimeout(params.guild, 0);
    ...
    function _distributeFunds(uint256 disputeId) internal {
        ...
        // Cache storage fields to local stack variables to save redundant SLOAD operations
        address escrowAddress = dispute.escrowAddress;
