...
contract GuildDAO is Initializable, AccessControlUpgradeable {
    ...
    function _addMember(address member) internal {
        // Caching the mapping lookup into a storage pointer avoids redundant keccak256 hash computations
    ...
    function removeMember(address member) external onlyRole(OFFICER_ROLE) {
        ...
        // Note: ADMIN_ROLE and DEFAULT_ADMIN_ROLE are not revoked here because
        // the function reverts early if the member has either of those roles.
