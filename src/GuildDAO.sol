...
contract GuildDAO is Initializable, AccessControlUpgradeable {
    ...
    function _addMember(address member) internal {
        ...
        if (m.isMember) revert AlreadyMember();
    ...
    function removeMember(address member) external onlyRole(OFFICER_ROLE) {
        ...
        // Revoke all roles
        // ⚡ Bolt Optimization: Removed redundant hasRole checks for DEFAULT_ADMIN_ROLE and ADMIN_ROLE.
        // The function already reverts early if the member has these roles, so checking again is dead code.
        // Avoiding these unreachable checks saves gas by skipping unnecessary mapping lookups.
