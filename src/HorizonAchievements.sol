...
contract HorizonAchievements is ERC721, ERC721URIStorage, ERC721Enumerable, AccessControl {
     ...
     * @param soulbound Whether the achievement is non-transferable
     * @param maxSupply Maximum number that can be minted (0 = unlimited)
     * @param tokenURIPrefix Base URI for this achievement type
    ...
    function createAchievementType(
        ...
        _achievementTypes[typeId] = AchievementType({
            ...
            maxSupply: maxSupply,
            currentSupply: 0,
            baseTokenURI: tokenURIPrefix,
            xpReward: xpReward
        });

        emit AchievementTypeCreated(typeId, name, category, soulbound, maxSupply);
    }
    ...
    function batchMintAchievements(
        ...
        /// @dev Validate the achievement type once up-front — mirrors the checks in
        ///      mintAchievement(). Without these guards a batch call could mint against
        ///      an unknown, inactive, or exhausted type, bypassing supply controls.
        AchievementType storage achievementType = _achievementTypes[typeId];
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            /// @dev Check maxSupply inside the loop so each mint respects the cap,
            ///      even when multiple recipients push currentSupply to the limit.
            if (achievementType.maxSupply > 0 &&
                achievementType.currentSupply >= achievementType.maxSupply) {
                revert MaxSupplyReached();
            }
