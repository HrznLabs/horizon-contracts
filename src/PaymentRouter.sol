...
contract PaymentRouter is AccessControl, Pausable, ReentrancyGuard, IPaymentRouter {
     ...
     * @dev Security: zero-address guard prevents guild fees from being silently transferred
     *      to the zero address on every payment settlement that routes through this guild.
    ...
    function _isFactoryEscrow(address caller) internal view returns (bool) {
        ...
        (bool success, bytes memory data) = factory.staticcall(
            abi.encodeWithSelector(IMissionFactory.getMissionByEscrow.selector, caller)
        );
