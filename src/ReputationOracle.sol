...
contract ReputationOracle is AccessControl, IReputationOracle {
    ...
    /// @notice Maximum number of users that can be updated in a single batch call.
    /// @dev Prevents out-of-gas DoS on batchUpdateScores(). At 500 entries the call
    ///      fits comfortably within the Base block gas limit (~15M gas).
