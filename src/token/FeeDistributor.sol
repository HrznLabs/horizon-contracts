    ...
    function distribute(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (totalGuildVolume > 0) {
            ...
            for (uint256 i = 0; i < len; i++) {
                ...
                // ⚡ Bolt: Merged volume reset into distribution loop to save O(N) gas overhead.
                // Placed outside `if (vol > 0)` to ensure all volumes are correctly cleared
                // in edge cases where guildTotal calculation prevents distribution but volume exists.
