...
contract FeeDistributor is AccessControl {
    ...
    function distribute(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        uint256 tGuildVolume = totalGuildVolume;
        if (tGuildVolume > 0) {
            ...
            for (uint256 i = 0; i < len; i++) {
                ...
                if (vol == 0) continue;
                uint256 guildShare = (guildTotal * vol) / tGuildVolume;
                if (guildShare > 0) {
                    usdc.safeTransfer(guild, guildShare);
                    emit GuildPaid(guild, guildShare);
                }
                delete guildVolume[guild]; // Clear volume inline to avoid a second loop
            }
            totalGuildVolume = 0;
        } else {
