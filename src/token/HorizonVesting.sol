...
contract HorizonVesting is Ownable {
    ...
    ///
    /// @dev ORDERING INVARIANT: `vestedAmount()` returns 0 whenever `revoked == true`.
    ///      Therefore the vested amount MUST be captured before `revoked` is set to
    ///      true, otherwise the calculation would incorrectly return zero and the full
    ///      contract balance would be swept to the treasury instead of leaving the
    ///      beneficiary's already-accrued share claimable.
    function revoke() external onlyOwner {
        require(!revoked, "HorizonVesting: already revoked");

        // Capture vested amount BEFORE flipping `revoked`. See ordering invariant above.
        uint256 vestedNow = vestedAmount(uint64(block.timestamp));
        uint256 releasableNow = vestedNow > released ? vestedNow - released : 0;
